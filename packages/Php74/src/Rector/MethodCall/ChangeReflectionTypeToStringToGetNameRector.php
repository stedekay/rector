...
PHP
    ...
    public function refactor(Node $node): ?Node
    {
        if ($node instanceof MethodCall) {
            $this->collectCallByVariable($node);

            if ($this->shouldSkipMethodCall($node)) {
                return null;
            }

            if ($this->isReflectionParameterGetTypeMethodCall($node)) {
                return $this->refactorReflectionParameterGetName($node);
            }

            if ($this->isReflectionFunctionAbstractGetReturnTypeMethodCall($node)) {
                return $this->refactorReflectionFunctionGetReturnType($node);
            }
